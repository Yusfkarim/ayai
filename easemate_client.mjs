...
async function main() {
  ...
  try {
    ...
    // ئەگەر وەڵام SSE نەبوو — JSON ـە (بۆ نموونە 6101 کۆتایی توکن)
    const ctype = res.headers.get('content-type') || '';
    if (!ctype.includes('event-stream')) {
      const txt = await res.text();
      try {
        const j = JSON.parse(txt);
        emit({ ok: false, code: j?.code, error: j?.message || ('non-SSE response: ' + txt.slice(0, 140)) });
      } catch {
        emit({ ok: false, error: 'non-SSE: ' + txt.slice(0, 140) });
      }
      return;
    }
